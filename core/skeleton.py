...
def career_lobby(dry_run_turn=False):
  ...
  try:
    while bot.is_bot_running:
      ...
      if lifecycle.handle_connection_error():
         non_match_count = 0
         continue
         
      if lifecycle.handle_date_changed():
         non_match_count = 0
         continue
         
      if lifecycle.check_career_completion():
         lifecycle.handle_career_completion()
         lifecycle.auto_start_career()
         non_match_count = 0
         continue

      if lifecycle.check_game_lobby():
         lifecycle.process_game_lobby()
         non_match_count = 0
         continue
      ...
      if matches.get("cancel", False):
        ...
        if clock_icon:
          debug("Lost race, clicking try again automatically.")
          sleep(1)
          retry_clicked = device_action.locate_and_click("assets/buttons/try_again_btn.png", min_search_time=get_secs(3))
          if retry_clicked:
            info("Lost race → pressed Try Again automatically. Waiting 5s before restarting race...")
            sleep(5)
            start_race()
            info("Race retry procedure completed.")
          else:
            warning("Lost race but couldn't find Try Again button.")
          non_match_count = 0
